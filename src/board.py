...
def sanitize_territories(territories: [Territory]):
    """
    Checks if all territory ID's are unique
    """
    ids = set()
...
def sanitize_continents(continents: [Continent]):
    """
    Checks if all continent ID's are unique
    """
    ids = set()
...
class Board:
    ...
    def place_armies(self, territory: Territory, player, armies: int):
        """
        Allows a player to place armies on one of it's owned territories
        """
...
class ClassicBoard(Board):
    def __init__(self, players: int):
        ...
        kamchatka = Territory(25, "Kamchatka", asia)
